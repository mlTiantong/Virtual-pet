...
public sealed class DefaultBehaviorScheduler
{
    public PetAction HandleTap(PetHitRegion region, int repeatCount, PetState state)
    {
        ...
        return region switch
        {
            ...
            PetHitRegion.Face => new PetAction("clasp_idle_m8", "tap.face"),
            PetHitRegion.Hand => new PetAction("hand_invite_m8", "tap.hand"),
            PetHitRegion.Body or PetHitRegion.Outfit => new PetAction("plush_hug_m8", "tap.outfit"),
            PetHitRegion.Accessory => new PetAction("plush_hug_m8", "tap.accessory"),
    ...
    {
        if (state.Energy < 24) return new PetAction("sleepy_m8", "idle.lowEnergy", BubbleSeconds: 3);
